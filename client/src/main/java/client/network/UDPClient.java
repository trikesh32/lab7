...
public class UDPClient {
    ...
    public UDPClient(InetAddress address, int port) throws IOException{
        ...
        logger.info("DatagrammChannel подключен к " + addr);
    }

    public Response sendAndReceiveCommand(Request request) throws IOException, TimeoutException {
    ...
    private byte[] receiveData() throws IOException, TimeoutException {
        var received = false;
    ...
    private byte[] receiveData(int bufferSize) throws IOException, TimeoutException {
        ...
        while(address == null) {
            if (System.currentTimeMillis() - startTime > TIMEOUT){
                ...
                throw new TimeoutException("Время ожидания вышло");
            }
    ...
    private byte[] sendAndReceiveData(byte[] data) throws IOException, TimeoutException {
        sendData(data);
        return receiveData();
    }
